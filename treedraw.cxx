...
extern void treerecs_callback(Fl_Widget *ob, void *data);
 ...
 */
{
  ...
  fd_nj_plot->rank = 0;
	...
	Fl_Window *w = new Fl_Window(665, 530);
	w->xclass(TREE_WINDOW);
	Fl_Group *g = new Fl_Group(0, 0, w->w()-2, 50);

  #ifdef __APPLE__
    extern int fl_mac_os_version;
    const bool treerecs_interface = (fl_mac_os_version >= 100900);
  #else
    const bool treerecs_interface = true;
  #endif
  Fl_Menu_Bar* menubar = new Fl_Menu_Bar(3, 3, treerecs_interface ? 215 : 130, 20);
  ...
  if (treerecs_interface) {
    static const Fl_Menu_Item recsitems[] = {
      {"Reconcile, root and rearrange with Treerecs", 0, treerecs_callback, NULL, 0},
    };
    fd_nj_plot->menu_recs = new vlength_menu(menubar, "Reconcile", recsitems, sizeof(recsitems)/sizeof(Fl_Menu_Item));
  }
...
static void copy(Fl_Widget *wgt, void *data)
{
  ...
  Fl_Surface_Device *copy_surf = NULL;
#if defined(__APPLE__) && defined(USE_PICT)
  if (fd_nj_plot->fd_unrooted == NULL) copy_surf = new Copy_Surface(g->w(), g->h());
#endif
  if (!copy_surf) copy_surf = new Fl_Copy_Surface(g->w(), g->h());
...
#ifdef unix
  setlocale(LC_NUMERIC, "C"); // use . as decimal point in branch lengths
#endif
