...
if(reponse >= 0 && reponse <  view->count_msa_algos) { 
  ...
  char tmp[5];
  sprintf(tmp, "%d", reponse);
  set_res_value("alignment", tmp);
  save_resources();
	}
}
...
int command_line_align(int argc, char **argv)
{
  ...
  if (need_back_translate) {
    ...
    view = DNAview;
    int maxl = 3*((view->seq_length+2)/3); // desired alignment length as multiple of 3
    for(i = 0; i < view->tot_seqs; i++) { // extend all seqs to maxl length with gaps
      int l = view->each_length[i];
      if (l == maxl) continue;
      p = (char *)realloc(view->sequence[i], maxl + 1);
      if (p == NULL) continue;
      view->sequence[i] = p;
      p += l;
      while(l++ < maxl) *(p++) = '-';
      view->each_length[i] = maxl;
      *p = 0;
      }
  }
