...
void geni_act(SEA_VIEW *view)
{
  ...
  double P, Psum=0., Q, Qsum=0., Pmin=0, Pmax=0, Qmin=0, Qmax=0, titvratio, titvmin, titvmax, freq[16], mat_obs[16], *ti, *tv;
  char **genseq, **genname, *text, *minname[4], *maxname[4], nt[4]={'A', 'C', 'G', 'T'};
  const char *titvminname1 = "", *titvminname2 = "";
  const char *titvmaxname1 = "", *titvmaxname2 = "";
  int lgseq = view->seq_length;
  ...
  if(!view->protein && nb>1 && lg!=0){
    ...
    double PQratio = Qsum ? (double)Psum/Qsum : 0;
    if(nb>2)
